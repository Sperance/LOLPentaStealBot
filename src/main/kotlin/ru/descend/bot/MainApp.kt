                ...
                if (ENABLE_POSTGRESQL) {
                    if (guildData != null && fireGuildTable.first { FireGuildTable::idGuild eq guildData.id } == null) {
                        FireGuildTable(
                            ...
                            botChannelId = guildData.botChannelId,
...
suspend fun createMessagePentaData(channelText: TextChannel, allMatches: ArrayList<FireMatch>, allPersons: ArrayList<FirePerson>, file: FireGuild) {
...
suspend fun createMessageGlobalStatistic(channelText: TextChannel, allMatches: ArrayList<FireMatch>, allPersons: ArrayList<FirePerson>, file: FireGuild, mapWins: HashMap<FirePerson, Int>) {
    val message = channelText.createMessage("Initial Message")
    channelText.getMessage(message.id).edit {
        editMessageGlobalStatisticContent(this, allMatches, allPersons, message.getGuild(), mapWins)
    }
...
suspend fun createMessageMastery(channelText: TextChannel, map: HashMap<FirePerson, ChampionMasteryDto>, allPersons: ArrayList<FirePerson>, file: FireGuild) {
    val message = channelText.createMessage("Initial Message")
...
fun editMessageMasteryContent(builder: UserMessageModifyBuilder, map: HashMap<FirePerson, ChampionMasteryDto>, allPersons: ArrayList<FirePerson>, guild: Guild) {
    ...
    val listHeroes = sortedMap.map { formatInt(allPersons.find { per -> per.LOL_puuid == it.key.LOL_puuid }!!.personIndex, 2) + "| " + LeagueMainObject.findHeroForKey(it.value.getOrNull(0)?.championId.toString()) + " / " + LeagueMainObject.findHeroForKey(it.value.getOrNull(1)?.championId.toString()) + " / " + LeagueMainObject.findHeroForKey(it.value.getOrNull(2)?.championId.toString()) }
...
fun editMessageGlobalStatisticContent(builder: UserMessageModifyBuilder, allMatches: ArrayList<FireMatch>, allPersons: ArrayList<FirePerson>, guild: Guild, mapWins: HashMap<FirePerson, Int>) {
    ...
    val listGames = dataList.map { formatInt(allPersons.find { per -> per.LOL_puuid == it.puuid }!!.personIndex, 2) + "| " + formatInt(it.statGames, 3) + charStr + formatInt(it.statWins, 3) + charStr + formatInt(((it.statWins.toDouble() / it.statGames.toDouble()) * 100).toInt(), 2) + "%" }
    val listAllKills = dataList.map { formatInt(allPersons.find { per -> per.LOL_puuid == it.puuid }!!.personIndex, 2) + "| " + it.kills.toFormatK() + charStr + formatInt(it.kills3, 2) + charStr + formatInt(it.kills4, 2) + charStr + formatInt(it.kills5, 2) }
...
fun editMessagePentaDataContent(builder: UserMessageModifyBuilder, allMatches: ArrayList<FireMatch>, allPersons: ArrayList<FirePerson>, guild: Guild) {
    val dataList = ArrayList<DataBasic>()
    allMatches.sortByDescending { it.matchDate }
    allMatches.forEach { match ->
        match.getParticipants(allPersons).forEach { perc ->
            match.listPerc.find { perc.LOL_puuid == it.puuid }?.let { firePart ->
                if (firePart.kills5 > 0 && dataList.size < 20) dataList.add(
                    DataBasic(user = perc, text = "Пентакилл за '${LeagueMainObject.findHeroForKey(firePart.championId.toString())}'", date = match.matchDate)
                )
            }
        }
    }
