...
class PostgreTest {
    ...
    {
        runBlocking {
            val base = R2DBC.db.withTransaction {
                R2DBC.db.runQuery {
                    QueryDsl.from(tbl_KORDLOLs)
                        .leftJoin(tbl_LOLs) { tbl_LOLs.id eq tbl_KORDLOLs.LOL_id }
                        .leftJoin(tbl_KORDs) { tbl_KORDs.id eq tbl_KORDLOLs.KORD_id }
                        .includeAll()
                }
            }
            base[tbl_KORDLOLs].forEachIndexed { index, kordloLs ->
                val lol = base[tbl_LOLs].elementAt(index)
                val kord = base[tbl_KORDs].elementAt(index)
                print("[KORDLOL] $kordloLs\n")
                print("[LOL] $lol\n")
                println("[KORD] $kord\n")
            }
        }
            ...
            db.withTransaction {
                db.runQuery {
                    QueryDsl.fromTemplate("select * from table_k_o_r_d__l_o_ls").select {
                        var kord = it.int("k_o_r_dperson")!!
                        var lol = it.int("l_o_lperson")!!
                        val guild = it.int("guild")!!
                        val id = it.int("id")!!
                        val mmrAram = it.double("mmr_aram")!!
                        val mmrAramSaved = it.double("mmr_aram_saved")!!
                        val data = KORDLOLs(
                            KORD_id = kord,
                            LOL_id = lol,
                            oldID = id,
                            guild_id = guild,
                            mmrAram = mmrAram,
                            mmrAramSaved = mmrAramSaved
                        )
                        kordList.add(data)
                    }
                }
                kordList.forEach {
                    var kordid = it.KORD_id
    ...
    fun testCatchData_participants() {
        runBlocking {
            ...
            db.withTransaction {
                ...
                kordList.forEach {
                    var lolid = it.LOLperson_id
                    it.LOLperson_id = R2DBC.getLOLs { tbl_LOLs.oldID eq lolid }.first().id
                    it.save()
                }
            }
        }
